...
<ParamField body="type" type="string" required>
  ...
  - `references` - A zip archive of reference recordings for a Voice lines video (up to 500 MB)
</ParamField>
...
<ParamField body="fragments" type="array">
  ...
  - `timePositionMs` - start time in milliseconds (must be >= 0)

  Optional trimming fields (audio only):
  - `timeInMs` - trim start position within the source file in milliseconds (must be >= 0). If omitted, trimming starts from the beginning
  - `timeOutMs` - trim end position within the source file in milliseconds (must be > timeInMs when both are provided). If omitted, trimming goes to the end

  When specified, only the matching portion is extracted from the source and placed at `timePositionMs` in the final artifact.

  All fragments must have the same format (codec, resolution, frame rate). Fragments are sorted by `timePositionMs` and gaps between them are filled with silence (audio) or black frames (video).

  **Use when:** Combining multiple clips (or portions of clips) into one file
</ParamField>

<ParamField body="artifactDurationMs" type="number">
  Target duration of the final artifact in milliseconds. Can only be used with `audio` type `fragments`.

  When specified, the concatenated result is adjusted to match this exact duration:
  - If the result is shorter, silence is appended
  - If the result is longer, it is trimmed

  **Use when:** You need the audio artifact to have a precise duration (e.g., matching a source video length)
</ParamField>
  ...
  -d '{
    ...
    "fragments": [
      ...
      },
      {
        ...
        "timePositionMs": 10000
      },
      {
        "url": "https://example.com/outro.mp4",
        "timePositionMs": 300000
      }
    ]
  }'
```

```bash Fragments (Audio with trimming)
curl -X POST \
  https://studio.dubformer.ai/api/v1/artifacts/create \
  -H 'Authorization: Bearer YOUR_API_KEY' \
  -H 'Content-Type: application/json' \
  -d '{
    "name": "Trimmed and Concatenated Audio",
    "type": "audio",
    "fragments": [
      {
        "url": "https://example.com/full-recording.wav",
        "timePositionMs": 0,
        "timeInMs": 5000,
        "timeOutMs": 15000
      },
...
```bash Fragments (Audio with exact duration)
curl -X POST \
  https://studio.dubformer.ai/api/v1/artifacts/create \
  -H 'Authorization: Bearer YOUR_API_KEY' \
  -H 'Content-Type: application/json' \
  -d '{
    "name": "Concatenated Audio",
    "type": "audio",
    "fragments": [
      {
        "url": "https://example.com/segment1.wav",
        "timePositionMs": 0
      },
      {
        "url": "https://example.com/segment2.wav",
        "timePositionMs": 120500
      }
    ],
    "artifactDurationMs": 360000
  }'
```
