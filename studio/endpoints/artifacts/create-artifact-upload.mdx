...
<ParamField body="fragments" type="array">
  ...
  All fragments must have the same format (codec, resolution, frame rate). Fragments are sorted by `startTime` and gaps between them are filled with silence (audio) or black frames (video).

  **Use when:** Combining multiple clips into one file
</ParamField>

<ParamField body="artifactDuration" type="number">
  Target duration of the final artifact in seconds. Can only be used with `audio` type `fragments`.
