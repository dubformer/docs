...
<Note>
  A Voice lines (`tts`) video has no transcription, translation or mixing. It reports `voiceover` while the
  script is uploaded and its voices are selected, then `rendering` while the lines are synthesized. A
  finished render rests at `pendingOn: "editor"`; a render that failed for some or all lines sets `isError`.
</Note>
