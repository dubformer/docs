...
<ParamField body="columns" type="object">
  **XLSX / XLS / CSV only.** Maps each script field to a column, addressed one of two ways:
  ...
  | `outputFileName` | **Voice lines.** Name of the delivered audio file. Lines sharing a name are joined into one file in script order; `.wav` is appended if missing. Without it every line becomes its own file. |
  | `inputFileName` | **Voice lines.** Name of the line's reference recording inside the references archive, matched by name or by name without extension. |
  | `speechPrompt` | **Voice lines.** Direction for the line (e.g. `calm, warm`). Becomes an inline tag for ElevenLabs v3 voices and a synthesis prompt elsewhere. |
  | `section`, `context`, `timeConstraints` | **Voice lines.** Notes shown to the editor. They do not affect synthesis. |
