---
title: 'Create Project'
api: 'POST https://studio.dubformer.ai/api/v1/projects/create'
description: 'Create a new project with general project information'
---

## Request Body

<ParamField body="name" type="string" required>
  Name of the project.
</ParamField>

<ParamField body="managerEmail" type="string">
  Email of the project manager. If not provided, the API key user will be used as the manager.
</ParamField>

<ParamField body="translationsGlossaryId" type="string">
  Optional ID of a translation glossary to be used for this project.
</ParamField>

<ParamField body="voicesGlossaryId" type="string">
  Optional ID of a voices glossary to be used for this project.
</ParamField>

<ParamField body="pronunciationGlossaryId" type="string">
  Optional ID of a pronunciation glossary to be used for this project.
</ParamField>

<ParamField body="folderId" type="string">
  Optional ID of an existing folder in your company. The project is created inside that folder. Omit this field to create the project at the company root.
</ParamField>

<ParamField body="remark" type="string">
  Optional remarks for the project.
</ParamField>

## Response

<ResponseField name="id" type="string">
  Unique project identifier.
</ResponseField>

<ResponseField name="name" type="string">
  Name of the project.
</ResponseField>

<ResponseField name="status" type="string">
  Current status of the project.
</ResponseField>

<ResponseField name="translationGlossary" type="string | null">
  ID of the translation glossary, if any.
</ResponseField>

<ResponseField name="voicesGlossary" type="string | null">
  ID of the voices glossary, if any.
</ResponseField>

<ResponseField name="pronunciationGlossary" type="string | null">
  ID of the pronunciation glossary, if any.
</ResponseField>

<ResponseField name="remark" type="string | null">
  Project remarks, if any.
</ResponseField>

<ResponseField name="clientName" type="string | null">
  Client name, if specified.
</ResponseField>

<ResponseField name="folderId" type="string | null">
  ID of the folder that contains the project, or `null` if the project is at the company root.
</ResponseField>

<ResponseField name="projectUrl" type="string">
  URL to access the project in the web app.
</ResponseField>

<ResponseField name="createdAt" type="string">
  ISO 8601 timestamp when the project was created.
</ResponseField>

<ResponseField name="updatedAt" type="string">
  ISO 8601 timestamp when the project was last updated.
</ResponseField>

<RequestExample>
```bash cURL
curl -X POST \
  https://studio.dubformer.ai/api/v1/projects/create \
  -H 'Authorization: Bearer YOUR_API_KEY' \
  -H 'Content-Type: application/json' \
  -d '{
    "name": "Marketing Campaign 2023",
    "managerEmail": "manager@company.com",
    "translationsGlossaryId": "gloss_5f83a14b2ec1b",
    "voicesGlossaryId": "voices_7c92e3a1f56d8",
    "pronunciationGlossaryId": "pron_3a91c7d2e84f5",
    "folderId": "folder_5f83a14b2ec1b"
  }'
```

```typescript TypeScript
import axios from 'axios';

const response = await axios.post(
  'https://studio.dubformer.ai/api/v1/projects/create',
  {
    name: "Marketing Campaign 2023",
    managerEmail: "manager@company.com",
    translationsGlossaryId: "gloss_5f83a14b2ec1b",
    voicesGlossaryId: "voices_7c92e3a1f56d8",
    pronunciationGlossaryId: "pron_3a91c7d2e84f5",
    folderId: "folder_5f83a14b2ec1b"
  },
  {
    headers: {
      'Authorization': 'Bearer YOUR_API_KEY',
      'Content-Type': 'application/json'
    }
  }
);

console.log(response.data);
```
</RequestExample>

<ResponseExample>
```json Response
{
  "id": "proj_5f83a14b2ec1b",
  "name": "Marketing Campaign 2023",
  "status": "new",
  "translationGlossary": "gloss_5f83a14b2ec1b",
  "voicesGlossary": "voices_7c92e3a1f56d8",
  "pronunciationGlossary": "pron_3a91c7d2e84f5",
  "remark": "Created via Dubformer Studio API",
  "clientName": null,
  "folderId": "folder_5f83a14b2ec1b",
  "projectUrl": "https://studio.dubformer.ai/projects/proj_5f83a14b2ec1b",
  "createdAt": "2023-12-01T10:00:00.000Z",
  "updatedAt": "2023-12-01T10:00:00.000Z"
}
```
</ResponseExample>

## Notes

<Note>
  This endpoint creates a project with basic information only. Videos can be added to the project using separate video management endpoints.
</Note>

<Note>
  The `managerEmail` must be a user from the same company as the API key owner. If not provided, the API key owner becomes the project manager.
</Note>

<Note>
  Translation, voices and pronunciation glossaries are optional but must exist and belong to your organization if provided.
</Note>

<Note>
  `folderId` must belong to the same company as the API key owner and must not be deleted. Omit it to keep the current behavior of creating the project at the company root.
</Note>